				...
				<div class="sale1">
					<c:if test="${empty slist}">
						...
						<table class="sale_tb1">
							...
							<tr>
								<td colspan="5">판매중인 상품이 없습니다.<br /> <input type="button"
									value="판매등록" onclick="location.href='./ProductReg.pro'">
								</td>
							</tr>
						</table>
					</c:if>
					...
					<c:if test="${!empty slist}">
						...
						<table class="sale_tb3">
							...
							<c:forEach var="slist" items="${slist }">
								<tr>
									<td><fmt:formatDate value="${slist.s_time }" type="both"
											pattern="YYYY'년' M'월' d'일'" /><br /> 상품No. ${slist.product_no }
									</td>
									<td><span> <a
											href="./ProductDetail.pro?product_no=${slist.product_no }">
												<c:if test="${slist.product_mainimg eq null}">
													<img class="thumbnail" src="http://placehold.it/100x100">
												</c:if> <c:if test="${slist.product_mainimg ne null}">
													<img class="thumbnail"
														src="./product_images_category/${slist.product_mainimg }">
												</c:if>
										</a>
									</span>
										<p>
											<a href="./ProductDetail.pro?product_no=${slist.product_no }">
												${slist.product_name } </a>
										</p> <a></a></td>
									<td>현재가 <fmt:formatNumber value="${slist.price_end }"
											type="number" /> 원 <br> (즉시구매가 <fmt:formatNumber
											value="${slist.price_promptly }" type="number" /> 원)<br>
									<br> 입찰수 0 회<br> (<fmt:formatDate
											value="${slist.e_time }" type="both"
											pattern="M'월'd'일' H'시'mm'분'" /> 마감)
									</td>
									<td
										onclick="window.open('./product/category_list_sellerinfo.jsp?seller_no='+${slist.seller_no }, '판매자 정보', 'width=621,height=200');">
										<c:if test="${slist.grade == 0}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade0.png">
										</c:if> <c:if test="${slist.grade == 1}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade1.png">
										</c:if> <c:if test="${slist.grade == 2}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade2.png">
										</c:if> <c:if test="${slist.grade == 3}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade3.png">
										</c:if> <c:if test="${slist.grade == 4}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade4.png">
										</c:if> <c:if test="${slist.grade == 5}">
											<img style="width: 50px; height: 50px;"
												src="./images/grade5.png">
										</c:if> <br>
									<a style="font-size: 15px; color: gray;">${slist.name }</a>
									</td>
								</tr>
							</c:forEach>
						</table>
